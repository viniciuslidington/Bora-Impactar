...
app.post("/repasse", async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        return res.status(500).json({ error: error.message || "Erro ao criar repasse" });
    }
});
...
app.get("/repasse", async (req, res) => {
    try {
      ...
      const requests = await prisma.relocationProduct.findMany({ where: { ong_Id: id } });
...
try {
    ...
    const requests = await prisma.relocationProduct.findMany({ where: filters });
    return res.status(200).json(requests);
} catch (error) {
    return res.status(500).json({ error: "Erro ao buscar solicitações" });
}
...
app.put("/repasse", async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        return res.status(500).json({ error: error.message || "Erro ao atualizar repasse" });
    }
});
